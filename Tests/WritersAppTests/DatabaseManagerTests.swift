...
final class DatabaseManagerTests: XCTestCase {
    ...
    func testNonSensitiveFieldsPersistCorrectly() throws {
        ...
        let temperature = try XCTUnwrap(retrieved).temperature
        XCTAssertEqual(temperature, 0.42, accuracy: 0.001, "temperature must persist in database")
    }
    ...
    func testNonAPIKeyConfigurationFieldsArePersistedCorrectly() throws {
        ...
        let temperature = try XCTUnwrap(retrieved).temperature
        XCTAssertEqual(temperature, 0.9, accuracy: 0.001, "Temperature must persist")
