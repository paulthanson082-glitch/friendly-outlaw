...
final class MockerKitTests: XCTestCase {
    ...
    func testImageInfoShortId() {
        // id starts with bare "sha256" (no colon), so PR branch applies: returns first 14 chars
        let image = ImageInfo(
    ...
    func testImageInfoShortId_sha256BarePrefix_fifteenChars_returns14() {
        let image = ImageInfo(
    ...
    func testImageInfoShortId_sha256BarePrefix_doesNotStripPrefix() {
        let image = ImageInfo(
    ...
    /// Regression: length for a long bare-prefix ID must be exactly 14.
    func testImageInfoShortId_sha256BarePrefix_lengthIsAlways14ForLongId() {
        let image = ImageInfo(
