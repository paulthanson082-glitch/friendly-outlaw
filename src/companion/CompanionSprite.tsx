...
const PET_BURST_MS = 2500 // how long hearts float after a pet interaction
...
function SpeechBubble({
  ...
  const borderColor = fading ? 'inactive' : (color as string)
  const bubble = (
    <Box
...
// Narrow viewports: 0 — REPL stacks the one-liner on its own row.
export function companionReservedColumns(
  terminalColumns: number,
  speaking: boolean,
): number {
  if (!getGlobalConfig().buddyEnabled) return 0
...
export function CompanionSprite(): React.ReactNode {
  ...
  useEffect(() => {
    const timer = setInterval(
      (setT: typeof setTick) => setT((t: number) => t + 1),
      TICK_MS,
      setTick,
    )
  ...
  if (!getGlobalConfig().buddyEnabled) return null
  const companion = getCompanion()
  ...
  // Narrow viewports: collapse to one-line face. When speaking, the quip
  ...
  // Name row doubles as hint row — unfocused shows dim name,
  // focused shows inverse name. flexShrink=0 stops the
  ...
  // Fullscreen: bubble renders separately via CompanionFloatingBubble.
...
// Floating bubble overlay for fullscreen mode. Mounted outside the
// overflow:hidden clip so it can extend into the scroll region.
// CompanionSprite owns the clear-after-10s timer; this just reads
// companionReaction and renders the fade.
export function CompanionFloatingBubble(): React.ReactNode {
  ...
  if (!getGlobalConfig().buddyEnabled || !reaction) return null
  const companion = getCompanion()
  if (!companion || getGlobalConfig().companionMuted) return null
