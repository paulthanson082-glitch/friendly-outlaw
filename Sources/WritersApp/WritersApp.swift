...
public class WritersApp {
    ...
    /// Update a prospect's status and, if the new status is a contacted-like state (`.contacted`, `.replied`, or `.meeting`), increment the active cowork session's `prospectsContacted` counter. The counter increments on every call with a contact status, regardless of the previous status.
    /// - Parameters:
