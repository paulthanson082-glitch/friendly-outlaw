...
public class HardwareManager {
    ...
    /// Gets a pin alias by name
    public func getPinAlias(boardId: UUID, aliasName: String) -> PinAlias? {
        return boards[boardId]?.pinAliases.first { $0.alias == aliasName }
    }
    ...
    /// Gets all peripherals for a board
    ...
    public func deletePeripheral(id: UUID) throws {
        guard peripherals[id] != nil else {
            throw HardwareError.peripheralNotFound
        }
    ...
    public func getBoardStatistics() -> (totalBoards: Int, byType: [BoardType: Int], activeCount: Int) {
        let byType = Dictionary(grouping: boards.values, by: { $0.boardType })
            .mapValues { $0.count }
        let activeCount = boards.values.filter { $0.metadata.isActive }.count
        return (boards.count, byType, activeCount)
    }
    ...
    private func loadDefaultBoards() {
        ...
        let arduinoUno = HardwareBoard(
            ...
            description: "Arduino Uno with ATmega328P, 14 digital pins, 6 analog inputs",
            serialPort: "/dev/ttyUSB0",
            baudRate: 9600,
            transportType: .serial,
            pinAliases: [
                PinAlias(alias: "red_led", physicalPin: 13, description: "Built-in LED", pinType: "digital"),
                PinAlias(alias: "green_led", physicalPin: 12, description: "Digital pin 12", pinType: "digital"),
                PinAlias(alias: "button", physicalPin: 2, description: "User button", pinType: "digital"),
                PinAlias(alias: "analog_sensor", physicalPin: 0, description: "Analog input A0", pinType: "analog"),
                PinAlias(alias: "temp_sensor", physicalPin: 1, description: "Analog input A1", pinType: "analog"),
                PinAlias(alias: "pwm_pin", physicalPin: 3, description: "PWM output pin 3", pinType: "pwm"),
                PinAlias(alias: "motor_pin", physicalPin: 9, description: "PWM motor control", pinType: "pwm"),
                PinAlias(alias: "soil_moisture_sensor", physicalPin: 2, description: "Analog A2", pinType: "analog")
            ],
            datasheetURL: "https://www.arduino.cc/en/Main/ArduinoBoardUno"
        )
        ...
        let nucleoF401RE = HardwareBoard(
            ...
            description: "STM32F401RE Arm Cortex-M4, 84 GPIO pins, 115200 baud",
            serialPort: "/dev/ttyACM0",
            baudRate: 115200,
            transportType: .serial,
            pinAliases: [
                PinAlias(alias: "user_led", physicalPin: 5, description: "User LED", pinType: "digital"),
